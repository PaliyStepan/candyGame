...
const App =  () => {
	...
	const checkForRowOfFour = () => {
		for (let i = 0; i <= 64; i++) {
			const rowOfTFour = [i, i + 1, i + 2, i + 3];
			const decidedColor = currentColorArrangement[i];
			...
			if (rowOfTFour.every(square => currentColorArrangement[square] === decidedColor  && !isBlank)) {
				setScoreDisplay((score) => score + 4);
				rowOfTFour.forEach(square => currentColorArrangement[square] = blank);
				return true;
			}
		}
	};
		...
		<div className="app">
			...
			<div className="game">
				...
				<div className="game__notice">
					Нужно собрать в колонку или в ряд, 3 или 4 конфеты
				</div>
